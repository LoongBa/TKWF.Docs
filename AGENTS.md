...
| 项 | 值 |
|:---|:---|
| **方案主文档** | `../_TKWF/docs/TKWF.Docs_设计文档/TKW.Framework-文档站设计方案-V1.0.md`（即 `F:\LoongBa_Git\_TKWF\docs\TKWF.Docs_设计文档\`） |
| **变更记录目录** | `../_TKWF/docs/TKWF.Docs_设计文档/变更记录/` |
| **版本命名** | `TKW.Framework-文档站设计方案-V{大}.{小}.{修订}.md` |
| **变更记录命名** | `TKWF.Docs-V{大}.{小}.{修订}-变更及审核记录.md` |
...
- **子版本（V1.0 / V1.1 / V1.2…）**：新增章节、内容调整、站点功能增强时升级。子版本升级**不删除**旧版本文件，而是新增带新版本号的文件（如 `V1.1`），保留 `V1.0` 供追溯。
- **修订（V1.0.1…）**：错别字、格式修正、术语同步等小改动，可直接修改当前版本文件。

### 1.3 每次子版本迭代必做

每次子版本迭代完成后，**必须**在 `变更记录/` 目录下编写一份变更记录文档：

```
TKWF.Docs-V1.x.x-变更及审核记录.md
```

**变更记录模板**：

```markdown
# TKWF.Docs 变更及审核记录 — V1.x.x

> 日期：YYYY-MM-DD · 状态：已实施/已审核/已发布
