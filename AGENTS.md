...
> **扩展模块文档跟踪位置（V4.9.80 起扩展独立仓库）**：
> - 扩展模块事实来源：`../_TKWF.Extensions/README.md`（扩展一览表：版本/状态/README/指南链接）与 `../_TKWF.Extensions/docs/`（各扩展使用指南）
> - 主仓库 `../_TKWF/docs/G17A-设计扩展模块-使用指南.md`（作者视角）/ `G17B-使用扩展模块-使用指南.md`（消费方视角）为**通用扩展机制指南**，非具体扩展指南
> - 扩展模块开发方案/ADR/总览跟踪仍在主框架私有 `../_TKWF/docs/03_扩展模块/`
> - 同步扩展相关站点文章时，先核对 `_TKWF.Extensions` 仓库（README + docs），再核对主仓库 D17/G17A/G17B
