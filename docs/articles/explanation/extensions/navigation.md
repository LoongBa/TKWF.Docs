...
description: 导航扩展使用指南：菜单项定义/贡献者、IMenuManager 树形组装 + 权限过滤（All/Any）、checker 缺失降级
---
...
`GetMenuAsync(string menuName)` 参数当前**忽略**（V4.9.74 单一菜单定义集）——`GetMenuAsync("Main")` 与 `GetMenuAsync("Admin")` 返回相同。多菜单组（Main/Admin/Mobile）是已规划的前瞻扩展点，注释已声明。
